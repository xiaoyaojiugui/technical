...
function check_container(){
    ...
    if [ -z "${container_exist}" ]; then
        echo "3、检查容器[${image_name}]不存在，初始化命令：docker run -d -p 3306:3306 -e MYSQL_ROOT_PASSWORD=root --name -v=${path}/conf/:/etc/mysql/conf.d/ -v=${path}/data:/var/lib/mysql ${image_alias} ${image_name}"
...
function mkdir_folder() {
    ...
    if [ ! -d ${path} ];then
		...
		if [ "$(uname)" == "Darwin" -o "$(expr substr $(uname -s) 1 10)" == "Linux" ];then
