...
function check_container(){
    ...
    if [ -z "$container_exist" ]; then
        mkdir_folder
        echo "3、检查容器[${image_name}]不存在，初始化命令：docker run -d -p 5432:5432 -e 'POSTGRES_PASSWORD=postgres' -e 'POSTGRES_DB=postgres' -v ${path}/data:/var/lib/postgresql/data -v ${path}/log:/var/log --name ${image_alias} ${image_name}"
