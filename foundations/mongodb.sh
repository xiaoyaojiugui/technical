...
if [ -z "$container_exist" ]; then
    echo "检查容器[${image_name}]不存在，初始化命令：docker run -d -p 27017:27017 --name ${image_alias} ${image_name}"
    docker run -d -p 27017:27017 --name ${image_alias} ${image_name}
else
    if [[ -z "$1" ]]; then
