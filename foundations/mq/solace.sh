...
function check_container(){
    ...
    if [ -z "$container_exist" ]; then
        echo "\n3、检查容器[${image_name}]不存在，初始化命令：sudo docker run -d -p 8080:8080 -p 55555:55555 --shm-size=1g --env username_admin_globalaccesslevel=admin --env username_admin_password=admin --name=solace solace/solace-pubsub-standard"
