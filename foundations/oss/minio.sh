...
function check_container(){
    ...
    if [ -z "$container_exist" ]; then
        echo "3、检查容器[${image_name}]不存在，初始化命令：docker run -d -p 9000:9000 --restart=always -v ${path}/data:/data -v ${path}/conf:/root/.minio --name ${image_alias} ${image_name}"
...
function mkdir_folder() {
    #这里的-d 参数判断$path是否存在 
    if [ ! -d $path ];then
		echo "2、创建文件夹，执行命令：sudo mkdir -p -v $path/{data,conf} && sudo chown -R $current_name $path/{data,conf}" 
		if [ "$(uname)" == "Darwin" ];then
