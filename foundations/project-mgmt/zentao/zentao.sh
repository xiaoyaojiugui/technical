...
function check_container() {
    ...
    if [[ -n "$1" ]]; then
        ...
        if [[ -z "${container_exist}" ]]; then
            echo "3、检查容器[${image_alias}]不存在，执行命令：docker run -d -p 8080:80 -e MYSQL_ROOT_PASSWORD=root -v ${path}/zentaopms:/www/zentaopms -v ${path}/mysqldata:/var/lib/mysql --name ${image_alias} ${image_name}"
            docker run -d -p 18080:80 -p 13306:3306 \
