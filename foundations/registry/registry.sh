...
image_name="registry:2.7"
image_alias="registry"
...
if [ -z "$container_exist" ]; then
    echo "检查容器[${image_name}]不存在，初始化命令：docker run -d -p 5000:5000 --name ${image_alias} ${image_name}"
    docker run -d -p 5000:5000 --name ${image_alias} ${image_name}
else
    if [[ -z "$1" ]]; then
