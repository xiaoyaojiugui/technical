...
if [ -z "$container_exist" ]; then
    echo "检查容器[${image_name}]不存在，初始化命令：docker run -d --name ${image_alias} ${image_name} --vm 1 --verbose"
    docker run --name ${image_alias} ${image_name} --vm 1 --verbose --cpu-shares=10  --cpu 1 --memory=200M --vm-bytes 500M
else
    if [[ -z "$1" ]]; then
