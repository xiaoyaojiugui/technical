...
function checkt_container_status (){
    echo "\n4、查看容器[${image_name}]状态，执行命令："
    echo "docker inspect ${image_alias} | jq -r '.[].State.Status'"
    status=$(docker inspect ${image_alias} | jq -r '.[].State.Status')

    if [ "$status" == "running" ]; then
        echo "\n4.1、查看容器[${image_name}]状态：[$status]"
    elif [ "$status" == "exited" ]; then
        echo "\n4.1、查看容器[${image_name}]状态：[$status]，执行命令：docker start ${image_alias}"
        docker start ${image_alias}
    else
        echo "\n4.1、查看容器[${image_name}]状态：[$status]，存在异常"
        exit 1 #强制退出，不执行后续步骤
    fi

    echo "\n4.2、查看容器[${image_name}]详情，执行命令："
...
function write_config() {
    if [ -z "$container_exist" ]; then
        echo "\n5、拷贝airflow.cfg到指定目录[${path}]，执行命令："
        ...
        echo "\n5.2、查看docker中mysql的IP地址：[$ipAddress]"
        
        echo "\n5.3、修改airflow.cfg配置中的executor和sql_alchemy_conn"
        ...
        echo "\n5.4、查看airflow的airflow.cfg配置是否修改成功"
...
function reset_container(){
    ...
    if [ -z "$container_exist" ]; then
        write_config
        echo "\n6、删除容器[${image_name}]，该操作用于环境调试"
        docker stop ${image_alias} && docker rm ${image_alias}

        echo "\n7、重新初始化容器[${image_name}]，初始化命令："
